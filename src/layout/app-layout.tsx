...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['meal-planner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<NutritionistInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/nutritionists` : null),
    () => getNutritionists({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Nutritionist',
      path: isTenantUser ? `/nutritionists/view/${data?.[0]?.id}` : '/nutritionists',
      entity: 'nutritionist',
